...
public class PurchaseInvoiceServiceImpl implements PurchaseInvoiceService {
	...
	public String getPurchaseInvoiceData(String purchaseInvoiceData) {
		...
		for (int i = 0; i < purchaseInvoiceDataObj.getJSONArray("JSMX").size(); i++) {
			...
			if(glDate==null || glDate.equals("")) {
				...
				try {
					conn = DBUtils.getConnection();
					if(conn!=null) {
						pst = conn.prepareStatement("select distinct top 1 RKJS_MIXNUMBER,JSMX_GYSBH,JSMX_GLRQ,JSMX_BZ,JSMX_HLV,JSMX_ZHKM,RKJS_ZY,RKJS_JSHJ,RKJS_BMBH,RKJS_ZDRYGBH,JSMX_JSKJ from t_ESB_PurchaseInvoice where IsRead=0 and id = (select MAX(id) from t_esb_PurchaseInvoice)");
						rs = pst.executeQuery();
						while (rs.next()) {
